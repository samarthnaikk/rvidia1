...
export function GoogleSignInButton({ role = "user" }: GoogleSignInButtonProps) {
  ...
  return (
    <Button
      ...
      disabled={signingIn}
      size="lg"
      className="w-full h-11 sm:h-12 bg-white/10 backdrop-blur-sm text-white border border-white/20 hover:bg-white/20 focus:ring-2 focus:ring-blue-400 transition-all text-sm sm:text-base"
    >
      {signingIn ? (
        <>
